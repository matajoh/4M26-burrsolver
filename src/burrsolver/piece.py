...
class Piece(NamedTuple("Piece", [("shape", int),
    ...
    Each piece is an instance of a shape at a paricular position and
    orientation.
    """
    ...
    def __str__(self) -> str:
        ...
        would be the same piece at (1, 2, 3) with X alignment.
        """
        o = chr(97 + self.orientation)
        if self.position == PLACES["A"]:
            return f"A{self.shape+1}{o}"
        if self.position == PLACES["B"]:
            return f"B{self.shape+1}{o}"
        if self.position == PLACES["C"]:
            return f"C{self.shape+1}{o}"
        if self.position == PLACES["D"]:
            return f"D{self.shape+1}{o}"
        if self.position == PLACES["E"]:
            return f"E{self.shape+1}{o}"
        if self.position == PLACES["F"]:
            return f"F{self.shape+1}{o}"

        return f"{self.position}{self.shape + 1}{o}"
